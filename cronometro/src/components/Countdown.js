...
class Countdown extends Component {
    ...
    render() {
        ...
        let segundos = ("0" + (Math.floor((timerTime / 1000) % 60) % 60)).slice(-2);
        let minutos = ("0" + Math.floor((timerTime / 60000) % 60)).slice(-2);
        let horas = ("0" + Math.floor((timerTime / 3600000) % 60)).slice(-2);
        ...
        return (
            <div className="Countdown">
                ...
                {timerOn === true && timerTime >= 1000 && (<button className="Button-stop" onClick={this.stopTimer}>Parar</button>)}
                {timerOn === false && (timerStart !== 0 && timerStart !== timerTime && timerTime !== 0) && (<button className="Button-start" onClick={this.startTimer}>Continuar</button>)}
                {(timerOn === false || timerTime < 1000) && (timerStart !== timerTime && timerStart > 0) && (<button className="Button-reset" onClick={this.resetTimer}>Reiniciar</button>)}
            </div>
        );
    }
}
